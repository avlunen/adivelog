...
public class OSTCProtocol100801Mk2 extends OSTCProtocol100801 {
    ...
    private static final VersionRange FIRMWARE_VERSIONS = new VersionRange(VersionRange.getVersion("1.50"),
            VersionRange.getVersion("1.70"));
    private static final String[] HASHKEYS = new String[] {};

    protected static final Feature CF34MK2 = new Feature("ostc_cf34mk2", ColorValue.class); // Color
                                                                                            // #
                                                                                            // Battery
    protected static final Feature CF35MK2 = new Feature("ostc_cf35mk2", ColorValue.class); // Color
                                                                                            // #
                                                                                            // Standard
    protected static final Feature CF36MK2 = new Feature("ostc_cf36mk2", ColorValue.class); // Color
                                                                                            // #
                                                                                            // Divemask
    protected static final Feature CF37MK2 = new Feature("ostc_cf37mk2", ColorValue.class); // Color
                                                                                            // #
                                                                                            // Warnings
    protected static final Feature CF39MK2 = new Feature("ostc_cf39mk2", RadioButtonValue.class); // Adjust
                                                                                                  // fixed
                                                                                                  // SP
    protected static final Feature CF40MK2 = new Feature("ostc_cf40mk2", RadioButtonValue.class); // Ceiling
                                                                                                  // warn
                                                                                                  // color
    protected static final Feature CF41MK2 = new Feature("ostc_cf41mk2", RadioButtonValue.class); // Mix
                                                                                                  // Type
                                                                                                  // icons
    protected static final Feature CF43MK2 = new Feature("ostc_cf43mk2", FifteenBitCustomFunction.class); // Depth
                                                                                                          // Warning
                                                                                                          // [mbar]
    protected static final Feature CF44MK2 = new Feature("ostc_cf44mk2", EightBitCustomFunction.class); // CNS
                                                                                                        // Warning
                                                                                                        // [%]
    protected static final Feature CF45MK2 = new Feature("ostc_cf45mk2", EightBitCustomFunction.class); // GF
                                                                                                        // Warning
                                                                                                        // [%]
    protected static final Feature CF46MK2 = new Feature("ostc_cf46mk2", EightBitCustomFunction.class); // ppO2
                                                                                                        // Warning
                                                                                                        // [cbar]
    protected static final Feature CF47MK2 = new Feature("ostc_cf47mk2", EightBitCustomFunction.class); // Velocity
                                                                                                        // Warning
                                                                                                        // [m/min]
    protected static final Feature CF48MK2 = new Feature("ostc_cf48mk2", EightBitCustomFunction.class); // Time
                                                                                                        // Offset
                                                                                                        // [s]
    ...
    @Override
    public void downloadSettings(StatusInterface status) throws PortNotFoundException, PortInUseException,
            UnsupportedCommOperationException, IOException, TransferException {
    ...
    protected Map<Feature, OSTCValue> parseSettings(byte[] data) {
        ...
        result.put(SALINITY, new RadioButtonValue(salinity, 100, new int[] { 100, 101, 102, 103, 104 },
                new String[] { "ostc_salinity_100", "ostc_salinity_101", "ostc_salinity_102", "ostc_salinity_103",
                        "ostc_salinity_104" }));
        ...
        result.put(DECOTYPE, new RadioButtonValue(decotype, 0, new int[] { 0, 1, 2, 3, 4, 5 }, new String[] {
                "ostc_deco_zhl16", "ostc_deco_gauge", "ostc_deco_const_ppo2", "ostc_deco_apnoe", "ostc_deco_zhl16_gf",
                "ostc_deco_const_ppo2_gf" }));
        ...
        def = ParseUtil.parseCFValue(data, offset);
        cur = ParseUtil.parseCFValue(data, offset + 2);
        result.put(CF31, new RadioButtonValue(cur, new int[] { 0, 1, 666 }, new String[] { "ostc_cf31_symbol",
                "ostc_cf31_voltage", "ostc_cf31_symbol" }));
    ...
    protected Map<Feature, OSTCValue> parseSettings2(byte[] data) {
        ...
        def = ParseUtil.parseCFValue(data, offset);
        cur = ParseUtil.parseCFValue(data, offset + 2);
        result.put(CF38, new RadioButtonValue(cur, def, new int[] { 0, 1 }, new String[] { "ostc_cf38_mm",
                "ostc_cf38_mmss" }));

        offset += 4;
        def = ParseUtil.parseCFValue(data, offset);
        cur = ParseUtil.parseCFValue(data, offset + 2);
        result.put(CF39MK2, new RadioButtonValue(cur, def, new int[] { 0, 1 }, new String[] { "ostc_cf39mk2_no",
                "ostc_cf39mk2_yes" }));

        offset += 4;
        def = ParseUtil.parseCFValue(data, offset);
        cur = ParseUtil.parseCFValue(data, offset + 2);
        result.put(CF40MK2, new RadioButtonValue(cur, def, new int[] { 0, 1 }, new String[] {
                "ostc_cf40mk2_normal_color", "ostc_cf40mk2_warn_color" }));

        offset += 4;
        def = ParseUtil.parseCFValue(data, offset);
        cur = ParseUtil.parseCFValue(data, offset + 2);
        result.put(CF41MK2, new RadioButtonValue(cur, def, new int[] { 0, 1 }, new String[] { "ostc_cf41mk2_no",
                "ostc_cf41mk2_yes" }));

        offset += 4;
        def = ParseUtil.parseCFValue(data, offset);
        cur = ParseUtil.parseCFValue(data, offset + 2);
        result.put(CF42, new RadioButtonValue(cur, def, new int[] { 0, 1 }, new String[] { "ostc_cf42_noblink",
                "ostc_cf42_blink" }));
    ...
    protected void store2(Map<Feature, OSTCValue> data, byte[] buffer) {
        ...
        if (cv != null) {
            ...
            buffer[offset + 1] = (byte) ((cv.defaultValue() / 256) & 0xff); // should
                                                                            // be
                                                                            // 0!
            buffer[offset + 2] = (byte) (cv.currentValue() & 0xff);
            buffer[offset + 3] = (byte) ((cv.currentValue() / 256) & 0xff); // should
                                                                            // be
                                                                            // 0!
        }
        ...
        offset += 4;
        rbv = (RadioButtonValue) data.get(CF39MK2);
        if (rbv != null) {
            buffer[offset] = (byte) (rbv.getSelectedValue() & 0xff);
            buffer[offset + 1] = (byte) ((rbv.getSelectedValue() / 256) & 0xff); // should
                                                                                 // be
        ...
        offset += 4;
        rbv = (RadioButtonValue) data.get(CF40MK2);
        if (rbv != null) {
            buffer[offset] = (byte) (rbv.getSelectedValue() & 0xff);
            buffer[offset + 1] = (byte) ((rbv.getSelectedValue() / 256) & 0xff); // should
                                                                                 // be
        ...
        offset += 4;
        rbv = (RadioButtonValue) data.get(CF41MK2);
        if (rbv != null) {
            buffer[offset] = (byte) (rbv.getSelectedValue() & 0xff);
            buffer[offset + 1] = (byte) ((rbv.getSelectedValue() / 256) & 0xff); // should
                                                                                 // be
    ...
    public String getName() {
        return "OSTC Mk.2 " + getFirmwareVersions();
    }
}
