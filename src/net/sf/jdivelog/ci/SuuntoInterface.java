...
public class SuuntoInterface implements ComputerInterface {
    ...
    public void prepareTransfer(StatusInterface status) throws NotInitializedException, InvalidConfigurationException,
            PortNotFoundException, PortInUseException, UnsupportedCommOperationException, IOException {
        if (properties == null) {
        ...
        commPort.setSerialPortParams(computermodel.getBaudRate(), computermodel.getNumDataBits(),
    ...
    private JDive createDive(byte[] profile) {
        ...
        gas.setPstart(String.valueOf(pc.convertPressure(Double.valueOf(2 * (profile[len - 6] & 0xff)))));
        ...
        GregorianCalendar gc = new GregorianCalendar(year, profile[len - 11] - 1, profile[len - 12], profile[len - 13],
                profile[len - 14]);
        ...
        for (int i = len - 15; i > 4; i--) {
            ...
            if (sample == 0x7a) {
                ...
                depth += sample;
                dive.addDepth(String.valueOf(c.convertAltitude(Double.valueOf(depth))));
            }
        }
        dive.addDepth("0.00");
        c = new UnitConverter(UnitConverter.SYSTEM_METRIC, UnitConverter.SYSTEM_SI);
        gas.setPend(String.valueOf(pc.convertPressure(Double.valueOf(2 * (profile[1] & 0xff)))));
        dive.setTemperature(c.convertTemperature(Double.valueOf(profile[3])));
        dive.setSurfaceTemperature(c.convertTemperature(Double.valueOf(profile[2])));
    ...
    private void initializeGas(Gas gas, int oxygen) {
        if (oxygen == 0) {
            ...
            gas.setHelium(0.0);
            gas.setOxygen(Double.valueOf(oxygen / 100.0));
            gas.setNitrogen((100.0 - oxygen) / 100.0);
        }
    }

    private SuuntoComputerType getCurrentModel() throws InvalidConfigurationException {
    ...
    public SuuntoComputerType readComputerModel(StatusInterface status) throws IOException, ChecksumException,
            CommunicationTimeoutException, InvalidConfigurationException {
        byte[] res = readMemory(status, (byte) 0x0, (byte) 0x24, (byte) 1);

        return SuuntoComputerType.getFromIdentifier(res[0]);
    }
    ...
    public byte[] readMemory(StatusInterface status, byte addrHigh, byte addrLow, byte len) throws IOException,
            ChecksumException, CommunicationTimeoutException {
        byte[] cmd = new byte[5];
        cmd[0] = 0x05;
        cmd[1] = addrHigh;
        cmd[2] = addrLow;
    ...
    public void writeMemory(StatusInterface status, byte addrHigh, byte addrLow, byte[] data) throws IOException,
            ChecksumException, CommunicationTimeoutException {
        prepareWrite(status);
    ...
    private void prepareWrite(StatusInterface status) throws CommunicationTimeoutException, IOException,
            ChecksumException {
        byte[] cmd = new byte[3];
        cmd[0] = 0x07;
        cmd[1] = (byte) 0xa5;
    ...
    private byte[] readDive(StatusInterface status, boolean firstDive) throws IOException, ChecksumException,
    ...
    private static class SunntoConfigurationPanel extends JPanel {
        ...
        //
        /**
         * Get a combo box with all Suunto computer models.
         * 
         * @return model list
         */
    ...
    private void test() throws PortNotFoundException, PortInUseException, UnsupportedCommOperationException,
            IOException, ChecksumException, CommunicationTimeoutException {
        ...
        if (it.hasNext()) {
            ...
            commPort.setSerialPortParams(2400, SerialPort.DataBits.DataBits_8, SerialPort.Parity.ODD,
