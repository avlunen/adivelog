...
import net.sf.jdivelog.gui.util.TraversableJTextArea;
import net.sf.jdivelog.model.*;
...
public class DiveDetailPanel extends JPanel implements ActionListener, ItemListener {
    ...
    private JDateChooser fieldDate = null;
    ...
    private TraversableJTextArea fieldComment = null;
    ...
    private TraversableJTextArea fieldEquipmentComment = null;
    ...
    public DiveDetailPanel(Window parentWindow, MainWindow mainWindow, JDive dive, Masterdata masterdata, boolean readonly) {
        ...
        this.logbookChangeNotifier = mainWindow != null ? mainWindow.getLogbookChangeNotifier() : null;
        initialize();
        loadData();
    }
    ...
    public void itemStateChanged(ItemEvent e) {
        if (e.getSource() == fieldSet) {
            ...
            if (name != null) {
                for (EquipmentSet equipmentSet : mainWindow.getLogBook().getMasterdata().getEquipmentSets()) {
                    if (name.equals(equipmentSet.getName())) {
                        getFieldSuit().setText(equipmentSet.getSuit());
                        getFieldGloves().setText(equipmentSet.getGloves());
                        getFieldWeight().setText(equipmentSet.getWeight());
                        ArrayList<Tank> tanks = getTankTableModel().getTanks();
                        Tank tank;
                        if (tanks.size() > 0) {
                            tank = tanks.get(0);
                            if (tank.getType() == null || "".equals(tank.getType())) {
                                tank.setType(equipmentSet.getTankType());
                            }
                            Gas gas = tank.getGas();
                            if (gas.getTankvolume() == null || gas.getTankvolume().equals(Double.valueOf(0))) {
                                UnitConverter c = new UnitConverter(UnitConverter.getDisplaySystem(), UnitConverter.getSystem(newDive.getUnits()));
                                gas.setTankvolume(c.convertVolume(Double.valueOf(equipmentSet.getTankVolume())));
                            }
    ...
    private void loadData() {
        ...
        if (oldDive.getDate() != null) {
            getFieldDate().setDate(newDive.getDate());
            getFieldTime().setText(TIMEFORMAT.format(newDive.getDate()));
        }
        else {
            getFieldDate().setDate(Calendar.getInstance().getTime());
    ...
    public void saveData() {
        ...
        try {
            newDive.setDiveNumber(Long.valueOf(fieldDiveNumber.getText()));
        }
        ...
        newDive.setDate(String.valueOf(year), String.valueOf(month), String
                .valueOf(day));
        try {
            date = TIMEFORMAT.parse(fieldTime.getText());
            gc.setTime(date);
    ...
    private void initialize() {        
        ...
        if (mainWindow != null) {
            for (Buddy buddy1 : mainWindow.getLogBook().getMasterdata().getBuddys()) {
                buddyDictionary.addEntry(buddy1.getFirstname() + " " + buddy1.getLastname()); //$NON-NLS-1$
            }
        }
        diveTypeDictionary = new DefaultDictionary();
        if (mainWindow != null) {
            for (DiveType diveType : mainWindow.getLogBook().getMasterdata().getDivetypes()) {
                diveTypeDictionary.addEntry(diveType.getDescription());
            }
        }
        diveActivityDictionary = new DefaultDictionary();
        if (mainWindow != null) {
            for (DiveActivity diveActivity : mainWindow.getLogBook().getMasterdata().getDiveactivities()) {
                diveActivityDictionary.addEntry(diveActivity.getDescription());
            }
        }
        divePlaceDictionary = new DefaultDictionary();
        countryDicitonary = new DefaultDictionary();
        if (mainWindow != null) {
            for (DiveSite diveSite : mainWindow.getLogBook().getMasterdata().getDiveSites()) {
                String place = diveSite.getSpot();
                String country = diveSite.getCountry();
        ...
        if (mainWindow != null) {
            for (Suit suit1 : mainWindow.getLogBook().getMasterdata().getSuits()) {
                suitDictionary.addEntry(suit1.getDescription());
            }
        }
        glovesDictionary = new DefaultDictionary();
        if (mainWindow != null) {
            for (GloveType gloveType : mainWindow.getLogBook().getMasterdata().getGloveTypes()) {
                glovesDictionary.addEntry(gloveType.getDescription());
            }
        }
    ...
    private JTextField getFieldCity() {
        if (fieldCity == null) {
            ...
            fieldCity.setFocusable(false);
        }
        return fieldCity;
    }

    private JDateChooser getFieldDate() {
        if (fieldDate == null) {
            fieldDate = new JDateChooser();
            fieldDate.setName("fieldDate"); //$NON-NLS-1$
            fieldDate.setEnabled(!readonly);
        }
        return fieldDate;
    }
    ...
    private JTextField getFieldCountry() {
        if (fieldCountry == null) {
            ...
            fieldCountry.setFocusable(false);
        }
        return fieldCountry;
    }
    ...
    private JTextArea getFieldComment() {
        if (fieldComment == null) {
            fieldComment = new TraversableJTextArea(10,2);
    ...
    private JPopupMenu getPopupMenuSuits() {
        if (popupMenuSuits == null) {
            ...
            for (String s : suitDictionary.getAll()) {
    ...
    private JPopupMenu getPopupMenuGloves() {
        if (popupMenuGloves == null) {
            ...
            for (String s : glovesDictionary.getAll()) {
                JMenuItem item = new JMenuItem(s);
                item.addActionListener(this);
    ...
    private JTextArea getFieldEquipmentComment() {
        if (fieldEquipmentComment == null) {
            fieldEquipmentComment = new TraversableJTextArea();
    ...
    private DiveProfile getDiveProfile() {
        if (diveProfile == null) {
            diveProfile = new DiveProfile(mainWindow != null ? mainWindow.getLogBook().getProfileSettings() : new ProfileSettings());
    ...
    private PictureTableModel getPictureTableModel() {
        if (pictureTableModel == null) {
            pictureTableModel = new PictureTableModel(mainWindow != null ? mainWindow.getStatusBar() : new StatusInterface() {
    ...
    private void addPictures() {
        ...
        if (ret == JFileChooser.APPROVE_OPTION) {
            ...
            for (File file : files) {
                if (!getPictureTableModel().containsFile(file)) {
                    Picture p = new Picture();
                    p.setFilename(file.getPath());
