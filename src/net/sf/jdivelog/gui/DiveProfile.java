...
public class DiveProfile extends JComponent {
   private static final BasicStroke PPO2_STROKE = new BasicStroke(2, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 1,
         new float[] { 1, 3 }, 0);
...

   {
      ...
      if (colors == null) {
         MIX_COLORS = new Color[] { Color.BLACK, Color.PINK, Color.MAGENTA, Color.ORANGE, Color.CYAN, Color.PINK,
               Color.RED, Color.DARK_GRAY, Color.YELLOW };
      }
      else {
   ...
   private final int LEFTLABEL_SIZE = 80;
   ...
   public void paint(Graphics g) {
      ...
      if (d.getWidth() > 0 && d.getHeight() > 0) {
         BufferedImage img = new BufferedImage(Double.valueOf(d.getWidth()).intValue(),
               Double.valueOf(d.getHeight()).intValue(), BufferedImage.TYPE_INT_ARGB);
         ...
         g.drawImage(img, 0, 0, Double.valueOf(d.getWidth()).intValue(), Double.valueOf(d.getHeight()).intValue(),
               profileSettings.getBackgroundColor(), this);
      }
   }
   ...
   private void initialize() {
      ...
      this.decoinfos = null;
      this.maxPPO2 = new UnitConverter(UnitConverter.SYSTEM_METRIC, UnitConverter.getDisplaySystem())
            .convertPressure(2.0);
      // set the icons for alarmposititions
      alarmImage = Toolkit.getDefaultToolkit()
            .getImage(getClass().getResource("/net/sf/jdivelog/gui/resources/icons/alarm.png")); //$NON-NLS-1$
      decoImage = Toolkit.getDefaultToolkit()
            .getImage(getClass().getResource("/net/sf/jdivelog/gui/resources/icons/deco.png")); //$NON-NLS-1$
      bookmarkImage = Toolkit.getDefaultToolkit()
            .getImage(getClass().getResource("/net/sf/jdivelog/gui/resources/icons/bookmark.png")); //$NON-NLS-1$
      surfaceImage = Toolkit.getDefaultToolkit()
            .getImage(getClass().getResource("/net/sf/jdivelog/gui/resources/icons/surface.png")); //$NON-NLS-1$
      attentionImage = Toolkit.getDefaultToolkit()
            .getImage(getClass().getResource("/net/sf/jdivelog/gui/resources/icons/alarm.png")); //$NON-NLS-1$
      slowImage = Toolkit.getDefaultToolkit()
            .getImage(getClass().getResource("/net/sf/jdivelog/gui/resources/icons/slow.png")); //$NON-NLS-1$
      decoceilingpassedImage = Toolkit.getDefaultToolkit()
            .getImage(getClass().getResource("/net/sf/jdivelog/gui/resources/icons/decoceilingpassed.png")); //$NON-NLS-1$
      endofImage = Toolkit.getDefaultToolkit()
            .getImage(getClass().getResource("/net/sf/jdivelog/gui/resources/icons/alarm.png")); //$NON-NLS-1$
      safetystopceilingpassedImage = Toolkit.getDefaultToolkit()
            .getImage(getClass().getResource("/net/sf/jdivelog/gui/resources/icons/alarm.png")); //$NON-NLS-1$
      alarmPositions = new HashMap<Rectangle, String>();

      if (this.dive.getDive() != null && this.dive.getDive().getSamples() != null) {
         for(Gas gas : this.dive.getDive().getGases()) addMix(gas);

         if (dive.getDive().getMode() == Dive.MODE_TIME) parseSamplesTimeMode();
         else parseSamplesDeltaMode();

         Double maxTime = this.maxTime;
         Double maxTemp = this.maxTemp;
         Double maxDepth = this.maxDepth;

         for(DiveProfile profile : comparisonProfiles) {
         ...
         if (maxDepth != null) {
            double maxDepthNorm = Math.ceil(maxDepth.doubleValue() / (GRID_Y * ALT_PRECISION))
                  * (GRID_Y * ALT_PRECISION);
   ...
   private void parseSamplesDeltaMode() {
      ...
      UnitConverter c = new UnitConverter(UnitConverter.getSystem(this.dive.getUnits()),
            UnitConverter.getDisplaySystem());
      double depth = 0.0;
      
      for(Sample s : this.dive.getDive().getSamples()) {
         if (s instanceof Depth) {
            ...
            if (this.maxDepth == null || this.maxDepth.doubleValue() < depth) {
               this.maxDepth = Double.valueOf(depth);
            }
            addDepth(c.convertTime(Double.valueOf(time)).doubleValue(), depth, subprofileId);
         }
            ...
            if (!comparisonProfile && profileSettings.isFillDepth()) {
               if (depth != 0.0) {
                  addDepth(c.convertTime(Double.valueOf(time)).doubleValue() + 0.01, 0.0, subprofileId);
               }
            }
            addGasSwitch(c.convertTime(Double.valueOf(time)).doubleValue() + 0.02, subprofileId);
            ...
            if (this.maxTemp == null || this.maxTemp.doubleValue() < temp) {
               this.maxTemp = Double.valueOf(temp);
            }
            addTemperature(c.convertTime(Double.valueOf(time)).doubleValue(), temp);
         }
            ...
            addAlarm(c.convertTime(Double.valueOf(time)).doubleValue(), (String) s.getValue());
         }
         else if (s instanceof PPO2) {
            PPO2 p = (PPO2) s;
            String sensor = p.getSensor();
            addPPO2(c.convertTime(Double.valueOf(time)).doubleValue(), c.convertPressure(p.getValue()), sensor);
         }
      }
      maxTime = c.convertTime(Double.valueOf(time));
   }
      ...
      for(Sample s : this.dive.getDive().getSamples()) {
         if (s instanceof Depth) {
            ...
            if (profileSettings.isFillDepth() && depth != 0.0) {
               addDepth(c.convertTime(Double.valueOf(time)).doubleValue() + 0.001, depth, subprofileId);
               addDepth(c.convertTime(Double.valueOf(time)).doubleValue() + 0.002, 0.0, subprofileId);
            }
            addGasSwitch(c.convertTime(Double.valueOf(time)).doubleValue() + 0.002, subprofileId);
            mixname = (String) s.getValue();
            addSubprofile(++subprofileId, mixname);
            if (profileSettings.isFillDepth() && depth != 0.0) {
               addDepth(c.convertTime(Double.valueOf(time)).doubleValue(), 0.0, subprofileId);
               addDepth(c.convertTime(Double.valueOf(time)).doubleValue() + 0.001, depth, subprofileId);
            }
         }
         else if (s instanceof Temperature) {
   ...
   private Rectangle getGraphCoords() {
      return new Rectangle(LEFTLABEL_SIZE, TOPLABEL_SIZE,
            Double.valueOf(getPreferredSize().getWidth() - LEFTLABEL_SIZE - RIGHTLABEL_SIZE).intValue(),
            Double.valueOf(getPreferredSize().getHeight() - BOTTOMLABEL_SIZE - TOPLABEL_SIZE).intValue());
   }
   ...
   private void drawNoProfileData(Graphics2D g2d) {
      ...
      g2d.drawString(message, (float) (getGraphCoords().getCenterX() - r2d.getCenterX()),
            (float) (getGraphCoords().getCenterY() - r2d.getCenterY()));
      g2d.rotate(-100, getGraphCoords().getCenterX(), getGraphCoords().getCenterY());
   ...
   private void drawGrid(Graphics2D g) {
      ...
      for (int i = 0; i <= GRID_X; i++) {
         int x = Double.valueOf(getGraphCoords().getX() + i * getGraphCoords().getWidth() / GRID_X).intValue();
         int y_from = Double.valueOf(getGraphCoords().getY()).intValue();
         int y_to = Double.valueOf(getGraphCoords().getY() + getGraphCoords().getHeight()).intValue();
      ...
      for (int i = 0; i <= GRID_Y; i++) {
         int y = Double.valueOf(getGraphCoords().getY() + i * getGraphCoords().getHeight() / GRID_Y).intValue();
         int x_from = Double.valueOf(getGraphCoords().getX()).intValue();
         int x_to = Double.valueOf(getGraphCoords().getX() + getGraphCoords().getWidth()).intValue();
         g.setColor(profileSettings.getGridColor());
      ...
      if (ppo2s != null) {
         g.setColor(profileSettings.getGridLabelColor());
         g.drawString("[" + UnitConverter.getDisplayPressureUnit() + "]",
               (int) (getGraphCoords().getX() + getGraphCoords().getWidth() + 50),
               (int) (getGraphCoords().getY() + (getGraphCoords().getHeight() / 2)));
      }
      g.setColor(profileSettings.getGridLabelColor());
      g.drawString("[" + UnitConverter.getDisplayTimeUnit() + "]",
            (int) (getGraphCoords().getX() + (getGraphCoords().getWidth() / 2)),
   ...
   private void drawTempColorLabel(Graphics2D g, int offsetx, int offsety) {
      if (profileSettings.isShowTemperature() && drawTemp) {
         ...
         String labelMin = (Double.valueOf(getMinTempRounded())).intValue() + " "
               + UnitConverter.getDisplayTemperatureUnit();
         String labelMax = (Double.valueOf(getMaxTempRounded())).intValue() + " "
               + UnitConverter.getDisplayTemperatureUnit();
         ...
         for (int i = 0; i < 10; i++) {
            g.setColor(calcTempColor(to + (getMaxTempRounded() - to) * Double.valueOf(i).doubleValue() / 10));
      ...
      if (alarms != null) {
         ...
         for(TimeAlarm alarm : alarms) {
            if (alarm.getAlarm().equals(Alarm.ALARM_DECO)) {
               ...
               int x = Double
                     .valueOf((getGraphCoords().getX()
                           + (alarm.getTime() * timeScale) * getGraphCoords().getWidth() / GRID_X) - width / 2)
                     .intValue();
...
*/
   private PositionColorIndex drawDepthGraphs(DiveProfile profile, Graphics2D g, PositionColorIndex offset,
         boolean nofill) {
      if (profile.gases != null) {
         //drawLabel(g, offset.position++, profile.getLabel()); // TODO remove label
   ...
   private void drawDepthGraph(DiveProfile profile, Graphics2D g, int subprofileId, Color c, boolean nofill) {
      ...
      if (depths != null) {
         ...
         for(TimeDepth td : depths) {
            if (td.isGasSwitch()) {
               drawPoly(g, c, nofill, p);
               p = createNewPoly(nofill);
            }
            else {
               int x = Double.valueOf(getGraphCoords().getX() + (td.getTime() * timeScale) * getGraphCoords().getWidth() / GRID_X).intValue();
               int y = Double.valueOf(getGraphCoords().getY() + (td.getDepth() * altScale) * getGraphCoords().getHeight() / GRID_Y).intValue();
               p.addPoint(x, y);
            }
         }
         drawPoly(g, c, nofill, p);
   ...
   private void drawTemperatureGraph(Graphics2D g) { // TODO look into improving
      if (!comparisonProfile && profileSettings.isShowTemperature()
            && (temperatures == null || temperatures.size() > 0)) {
         if (dive.getTemperature() != null && dive.getSurfaceTemperature() != null) {
            drawTemp = true;
            UnitConverter uc = new UnitConverter(UnitConverter.getSystem(dive.getUnits()),
                  UnitConverter.getDisplaySystem());
      ...
      if (!comparisonProfile && profileSettings.isShowTemperature() && drawTemp && temperatureSegments != null) {
         int x0 = Double.valueOf(getGraphCoords().getX()).intValue();
         int w = Double.valueOf(getGraphCoords().getWidth()).intValue();
         int h = Double.valueOf(Math.ceil(getGraphCoords().getHeight() / segments)).intValue();
         for (int i = 0; i < temperatureSegments.length; i++) {
            int y = Double.valueOf(getGraphCoords().getY() + i * h).intValue();
            if (y > getGraphCoords().getHeight()) h = ((int) getGraphCoords().getHeight() - (i * h));
   ...
   private void drawPPO2Graph(Graphics2D g) {
      if (!comparisonProfile && profileSettings.isShowPpo2() && ppo2s != null && ppo2s.size() > 0) {
         ...
         Color[] colors = new Color[] { profileSettings.getPpo2Color1(), profileSettings.getPpo2Color2(),
               profileSettings.getPpo2Color3(), profileSettings.getPpo2Color4() };
         int count = 0;
         for (Entry<String, TreeSet<TimeDepth>> entry : ppo2s.entrySet()) {
            ...
            for (TimeDepth td : tds) {
               int x = Double
                     .valueOf(
                           getGraphCoords().getX() + (td.getTime() * timeScale) * getGraphCoords().getWidth() / GRID_X)
                     .intValue();
               int y = Double.valueOf(getGraphCoords().getY() + getGraphCoords().getHeight()
                     - (td.getDepth() * ppo2Scale) * getGraphCoords().getHeight() / GRID_Y).intValue();
               p.addPoint(x, y);
   ...
   private Color calcTempColor(double temp) {
      return ColorShadingUtil.calculate(temp, getMinTempRounded(), getMaxTempRounded(),
            profileSettings.getTemperatureColorBegin(), profileSettings.getTemperatureColorEnd());
   }
   ...
   private void drawDecoGraph(Graphics2D g) {
      if (!comparisonProfile && drawDeco && decoinfos != null && decoinfos.size() > 0) {
         ...
         int lasty = Double.valueOf(getGraphCoords().getY()).intValue();
         while (it.hasNext()) {
...
/*
   private String getLabel() {
      return dive.getDate() == null ? "" : DateFormatUtil.getDateTimeFormat().format(dive.getDate());
   }
*/
