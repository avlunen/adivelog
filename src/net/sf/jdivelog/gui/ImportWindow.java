...
public class ImportWindow extends JDialog implements ActionListener {
    ...
    public ImportWindow(MainWindow mainWindow, Set<JDive> dives) {
    ...
    /**
     * Selects all dives which are not ignored and are not already in the
     * logbook
     */
    private void updateSelection() {
        ...
        for (int i = 0; i < rowCount; i++) {
            ...
            String key = dive.getDate() == null ? null : dateFormat.format(dive.getDate());
            if (!existing.containsKey(key) && !ignored.containsKey(key)) {
    ...
    private void editDive(JDive dive) {
        DiveDetailWindow ddw = new DiveDetailWindow(this, this.mainWindow, dive, this.mainWindow != null ? this.mainWindow.getLogBook().getMasterdata() : null, true);
        ddw.setVisible(true);
    }
