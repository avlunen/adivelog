...
public class HtmlExportUtil {
   ...
   private static final SimpleDateFormat FORMAT_NICEDATE = new SimpleDateFormat(
         Messages.getString("export.nice_date_format")); //$NON-NLS-1$
   ...
   public int execute() throws ExportException {
      ...
      if (!outputDir.isDirectory() || !outputDir.canWrite()) {
         throw new ExportException(
               Messages.getString("export.error.could_not_write_to_dir") + " '" + outputDir.getPath() + "'"); //$NON-NLS-1$
      }
         ...
         try {
            ...
            if (logBook.getExportSettings().isFullExport() || !dive.isHtmlExported()) {
               if (logBook.getExportSettings().showDetailProfile()
                     || logBook.getExportSettings().getIndexImages() == 1) {
   ...
   private void addIndexEntry(int count, JDive dive) {
      ...
      if (logBook.getExportSettings().getIndexImages() == 1) {
         sb.append("      <div class='diveindexcell-" + getOddEvenString(count)
               + "' id='thumb'><img class='thumb' src='" + count + "/profile_thumb.jpg'></div>\n");
      }
      else if (logBook.getExportSettings().getIndexImages() == 2) {
         if (dive.getPictures() != null && dive.getPictures().size() > 0) {
            sb.append("      <div class='diveindexcell-" + getOddEvenString(count)
                  + "' id='thumb'><img class='thumb' src='" + count + "/image0_thumb.jpg'></div>\n");
         }
         else {
      ...
      if (logBook.getExportSettings().showIndexDate()) {
         if (dive.getDate() != null) {
            sb.append("      <div class='diveindexcell-" + getOddEvenString(count) + "' id='date'>"
                  + FORMAT_DATE.format(dive.getDate()) + "</div>\n");
         }
         else {
      ...
      if (logBook.getExportSettings().showIndexTime()) {
         if (dive.getDate() != null) {
            sb.append("      <div class='diveindexcell-" + getOddEvenString(count) + "' id='time'>"
                  + FORMAT_TIME.format(dive.getDate()) + "</div>\n");
         }
         else {
      ...
      if (logBook.getExportSettings().showIndexLocation()) {
         if (site != null && site.getSpot() != null) {
            sb.append("      <div class='diveindexcell-" + getOddEvenString(count) + "' id='spot'>" + site.getSpot()
                  + "</div>\n");
         }
         else {
      ...
      if (logBook.getExportSettings().showIndexCity()) {
         if (site != null && site.getCity() != null) {
            sb.append("      <div class='diveindexcell-" + getOddEvenString(count) + "' id='city'>" + site.getCity()
                  + "</div>\n");
         }
         else {
      ...
      if (logBook.getExportSettings().showIndexCountry()) {
         if (site != null && site.getCountry() != null) {
            sb.append("      <div class='diveindexcell-" + getOddEvenString(count) + "' id='country'>"
                  + site.getCountry() + "</div>\n");
         }
         else {
      ...
      if (logBook.getExportSettings().showIndexDepth()) {
         if (dive.getDepth() != null) {
            sb.append("      <div class='diveindexcell-" + getOddEvenString(count) + "' id='depth'>"
                  + DECIMALFORMAT.format(c.convertAltitude(dive.getDepth())) + " "
                  + UnitConverter.getDisplayAltitudeUnit() + "</div>\n");
         }
         else {
      ...
      if (logBook.getExportSettings().showIndexDuration()) {
         if (dive.getDuration() != null) {
            sb.append("      <div class='diveindexcell-" + getOddEvenString(count) + "' id='duration'>"
                  + DECIMALFORMAT.format(c.convertTime(dive.getDuration())) + " " + UnitConverter.getDisplayTimeUnit()
                  + "</div>\n");
         }
         else {
   ...
   private StringBuffer getIndexPage(JDive dive) {
      if (logBook.getExportSettings().groupByYear()) {
         ...
         if (indexPage == null) {
            ...
            indexPage.append(
                  "<a class='menulink' id='go_to_index' href='index.html'><div class='menuitem'><div id='tl'><div id='tr'><div id='br'><div id='bl'>zur&uuml;ck zur &Uuml;bersicht</div></div></div></div></div></a>\n");
            ...
            if (logBook.getExportSettings().getIndexImages() != 0) {
               indexPage.append(
                     "<div class='diveindexheading-cell' id='thumb'>" + Messages.getString("image") + "</div>\n");
            }
            if (logBook.getExportSettings().showDetailDate()) {
               indexPage
                     .append("<div class='diveindexheading-cell' id='date'>" + Messages.getString("date") + "</div>\n");
            }
            if (logBook.getExportSettings().showIndexTime()) {
               indexPage
                     .append("<div class='diveindexheading-cell' id='time'>" + Messages.getString("time") + "</div>\n");
            }
            if (logBook.getExportSettings().showIndexLocation()) {
               indexPage
                     .append("<div class='diveindexheading-cell' id='spot'>" + Messages.getString("spot") + "</div>\n");
            }
            if (logBook.getExportSettings().showIndexCity()) {
               indexPage
                     .append("<div class='diveindexheading-cell' id='city'>" + Messages.getString("city") + "</div>\n");
            }
            if (logBook.getExportSettings().showIndexCountry()) {
               indexPage.append(
                     "<div class='diveindexheading-cell' id='country'>" + Messages.getString("country") + "</div>\n");
            }
            if (logBook.getExportSettings().showIndexDuration()) {
               indexPage.append(
                     "<div class='diveindexheading-cell' id='duration'>" + Messages.getString("duration") + "</div>\n");
            }
            if (logBook.getExportSettings().showIndexDepth()) {
               indexPage.append(
                     "<div class='diveindexheading-cell' id='depth'>" + Messages.getString("depth") + "</div>\n");
            }
            if (logBook.getExportSettings().showIndexPictureCount()) {
               indexPage.append(
                     "<div class='diveindexheading-cell' id='pics'>" + Messages.getString("images") + "</div>\n");
            }
            indexPage.append("</div>\n");
   ...
   private void startIndexPage() {
      ...
      if (logBook.getExportSettings().groupByYear()) {
         ...
         if (logBook.getExportSettings().getIndexImages() != 0) {
            indexPage
                  .append("<div class='diveindexheading-cell' id='thumb'>" + Messages.getString("image") + "</div>\n");
         }
         if (logBook.getExportSettings().showDetailDate()) {
         ...
         if (logBook.getExportSettings().showIndexDepth()) {
            indexPage
                  .append("<div class='diveindexheading-cell' id='depth'>" + Messages.getString("depth") + "</div>\n");
         }
         if (logBook.getExportSettings().showIndexPictureCount()) {
            indexPage
                  .append("<div class='diveindexheading-cell' id='pics'>" + Messages.getString("images") + "</div>\n");
         }
         indexPage.append("</div>\n");
      }
   }
   ...
   private String getPageFooter() {
      ...
      boolean link = logBook.getExportSettings().getFooterLink() != null
            && !"".equals(logBook.getExportSettings().getFooterLink());
      boolean linkName = logBook.getExportSettings().getFooterLinkName() != null
            && !"".equals(logBook.getExportSettings().getFooterLinkName());
      if (link && linkName) {
         sb.append("<a href='");
   ...
   private void createProfile(int i, JDive dive) throws ExportException {
      ...
      try {
         ...
         throw new ExportException(
               Messages.getString("export.error.could_not_write_profile_images") + " " + diveDir.getPath()); //$NON-NLS-1$ //$NON-NLS-2$
      }
   ...
   private void createDetailsPage(int diveNumber, JDive dive) throws ExportException {
      ...
      try {
         ...
         throw new ExportException(
               Messages.getString("export.error.could_not_write_details_html") + " " + diveDir.getPath()); //$NON-NLS-1$ //$NON-NLS-2$
      }
   }
   ...
   private void createPicturesPage(int diveNumber, JDive dive) throws ExportException {
      ...
      if (pics != null) {
         ...
         while (it.hasNext()) {
            Picture pic = it.next();
            if ((logBook.getExportSettings().isFullExport() && !logBook.getExportSettings().isKeepImages())
                  || !dive.isHtmlExported()) {
      ...
      try {
         ...
         throw new ExportException(
               Messages.getString("export.error.could_not_write_pictures_html") + " " + diveDir.getPath()); //$NON-NLS-1$ //$NON-NLS-2$
      }
   }
   ...
   private String getPicturesPageHeader(int diveNumber, JDive dive) {
      ...
      if (logBook.getExportSettings().isDetailVisible()) {
         sb.append(
               "<a class='menulink' id='go_to_details' href='details.html'><div class='menuitem'><div id='br'><div id='tr'><div id='tl'><div id='bl'>");
         sb.append(Messages.getString("export.data"));
         sb.append("</div></div></div></div></div></a>\n");
      }

      sb.append(
            "<a class='menulink' id='go_to_pictures' href='pictures.html'><div class='menuitem'><div id='br'><div id='tr'><div id='tl'><div id='bl'>");
   ...
   private String getDetailsPageHeader(int diveNumber, JDive dive) {
      ...
      if (logBook.getExportSettings().isPictureVisible() && dive.getPictures() != null
            && dive.getPictures().size() > 0) {
         sb.append(
               "<a class='menulink' id='go_to_pictures' href='pictures.html'><div class='menuitem'><div id='br'><div id='tr'><div id='tl'><div id='bl'>");
         sb.append(Messages.getString("export.pictures"));
         sb.append("</div></div></div></div></div></a>\n");
   ...
   private void createPicturePage(int diveNumber, int pictureNumber, JDive dive, Picture picture, int numberOfImages)
      ...
      sb.append(
            "<a class='menulink' id='go_to_index' href='pictures.html'><div class='menuitem' id='go_to_index'><div id='tl'><div id='tr'><div id='br'><div id='bl'>Bilder &Uuml;bersicht</div></div></div></div></div></a>\n");
      ...
      if (logBook.getExportSettings().showPictureDescription() && picture.getDescription() != null
            && !"".equals(picture.getDescription())) {
      ...
      try {
         ...
         throw new ExportException(Messages.getString("export.error.could_not_write_picture_page_for_picture") + " " //$NON-NLS-1$ //$NON-NLS-2$
               + pictureNumber + " " + Messages.getString("export.of_dive") + " " + diveNumber); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      }
   }
   ...
   private void createPicture(int diveNumber, int pictureNumber, Picture picture) throws ExportException {
      ...
      try {
         writeScaledJPG(img, imgMaxX, imgMaxY, picture.getRotation(), imgFile,
               logBook.getExportSettings().isWatermarkEnabled());
         ...
         throw new ExportException(
               Messages.getString("export.error.could_not_write_images_to") + " " + diveDir.getPath()); //$NON-NLS-1$ //$NON-NLS-2$
      }
   }

   /**
    * @param img             The image to save
    * @param imgMaxX         maximum width of the image
    * @param imgMaxY         maximum height of the image
    * @param imgRotation     totation of the image (0=0°, 1=90°, 2=180°, 3=270°)
    * @param file            the file which should be written
    * @param createWatermark whether a watermark should be created or not
    * @throws IOException an IOException if the Image could not be saved.
    */
   private void writeScaledJPG(Image img, int imgMaxX, int imgMaxY, int imgRotation, File file, boolean createWatermark)
         throws IOException {
      ...
      if (createWatermark) {
         ...
         g.drawString(logBook.getExportSettings().getWatermarkText(),
               scaledImg.getWidth() - (int) wmShape.getWidth() - border,
               scaledImg.getHeight() - (int) wmShape.getHeight());
         g.dispose();
      }
      ...
      while (it.hasNext()) {
         ...
         if (pic.getFilename() != null) {
            ...
            if (f.exists() && f.canRead()) {
               ...
               LOGGER.warning("skipping pic '" + pic.getName() + "' (" + pic.getFilename()
                     + "). File does not exists or could no be read!");
            }
         }
         else {
