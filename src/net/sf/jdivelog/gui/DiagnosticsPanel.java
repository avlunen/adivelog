   ...
   public void load() {
         ...
         if (!f.exists()) {
            ...
            for (int i = 0; i < libs.length; i++) {
               ...
               if (fn != null && fn.toLowerCase().indexOf("rxtx") > -1) {
                  sb.append("Possible RXTX Lib found: " + libs[i].getAbsolutePath() + " (" + libs[i].length()
                        + " bytes, read:" + libs[i].canRead() + ")\n");
               }
            }
         }
      }
         ...
         if (!dir.exists()) {
            ...
            for (int i = 0; i < jars.length; i++) {
               ...
               if (fn != null && fn.toLowerCase().indexOf("comm") > -1) {
                  sb.append("Possible Comm Jar found: " + jars[i].getAbsolutePath() + " (" + jars[i].length()
                        + " bytes, read:" + jars[i].canRead() + ")\n");
               }
            }
         }
      }
      ...
      if (skinpath != null) {
         ...
         if (skindir.exists() && skindir.canRead() && skindir.isDirectory()) {
            sb.append("skindir content: " + skindir.list() == null || skindir.list().length == 0 ? null
                  : Arrays.asList(skindir.list()) + "\n");
         }
      }
