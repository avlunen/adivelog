...
public class StatisticPanel extends JPanel implements ActionListener {
   ...
   private JFreeChart getBuddyStatisticChart() {
      return getChart(mainWindow.getLogBook().getStatisticSettings().getBuddyStatistic().getType(),
            mainWindow.getLogBook().getStatisticSettings().getBuddyStatistic().getOrientation(), getBuddyData());
   }

   private JFreeChart getDivePlaceStatisticChart(String country) {
      return getChart(mainWindow.getLogBook().getStatisticSettings().getDivePlaceStatistic().getType(),
            mainWindow.getLogBook().getStatisticSettings().getDivePlaceStatistic().getOrientation(),
            getPlaceData(country));
   }

   private JFreeChart getCountryStatisticChart() {
      return getChart(mainWindow.getLogBook().getStatisticSettings().getCountryStatistic().getType(),
            mainWindow.getLogBook().getStatisticSettings().getCountryStatistic().getOrientation(), getCountryData());
   }

   private JFreeChart getDiveTypeStatisticChart() {
      return getChart(mainWindow.getLogBook().getStatisticSettings().getDiveTypeStatistic().getType(),
            mainWindow.getLogBook().getStatisticSettings().getDiveTypeStatistic().getOrientation(), getDiveTypeData());
   }

   private JFreeChart getDiveActivityStatisticChart() {
      return getChart(mainWindow.getLogBook().getStatisticSettings().getDiveActivityStatistic().getType(),
            mainWindow.getLogBook().getStatisticSettings().getDiveActivityStatistic().getOrientation(),
            getDiveActivityData());
   }

   private JFreeChart getWatersStatisticChart() {
      return getChart(mainWindow.getLogBook().getStatisticSettings().getWatersStatistic().getType(),
            mainWindow.getLogBook().getStatisticSettings().getWatersStatistic().getOrientation(), getWatersData());
   }
   ...
   private JFreeChart getChart(String type, String orientation, ChartData data) {
      try {
         ...
         if (TYPE_PIE.equals(type)) {
            ...
            JFreeChart chart = ChartFactory.createBarChart(data.getName(), data.getCatLabel(), data.getValLabel(),
                  dataset, getOrientation(orientation), false, true, false);
            // Post processing
   ...
   private JFreeChart getChart(CountChartData3D data) {
      try {
         status.messageInfo(Messages.getString("creating_chart"));
         status.infiniteProgressbarStart();
         CategoryDataset dataset = DatasetFactory.getCategoryDataset(data);
         JFreeChart chart = ChartFactory.createBarChart(data.getName(), data.getValLabel(), data.getCatLabel(), dataset,
   ...
   private ChartData getBuddyData() {
      ChartData data = new ChartData(Messages.getString("buddystatistic"), Messages.getString("buddy"),
            Messages.getString("dives"));
   ...
   private ChartData getCountryData() {
      ChartData data = new ChartData(Messages.getString("countrystatistic"), Messages.getString("country"),
            Messages.getString("dives"));
      status.messageInfo(Messages.getString("calculate_statistic"));
      status.countingProgressbarStart(divelog.getDives().size(), true);
      try {
   ...
   private ChartData getPlaceData(String country) {
      ChartData data = new ChartData(Messages.getString("diveplacestatistic") + " - " + country,
            Messages.getString("place"), Messages.getString("dives"));
      status.messageInfo(Messages.getString("calculate_statistic"));
      status.countingProgressbarStart(divelog.getDives().size(), true);
      try {
         Iterator<JDive> it = divelog.getDives().iterator();
   ...
   private ChartData getDiveTypeData() {
      ChartData data = new ChartData(Messages.getString("divetypestatistic"), Messages.getString("divetype"),
            Messages.getString("dives"));
      status.messageInfo(Messages.getString("calculate_statistic"));
      status.countingProgressbarStart(divelog.getDives().size(), true);
      try {
   ...
   private ChartData getDiveActivityData() {
      ChartData data = new ChartData(Messages.getString("diveactivitystatistic"), Messages.getString("diveactivity"),
            Messages.getString("dives"));
      status.messageInfo(Messages.getString("calculate_statistic"));
      status.countingProgressbarStart(divelog.getDives().size(), true);
      try {
   ...
   private ChartData getWatersData() {
      ChartData data = new ChartData(Messages.getString("watersstatistic"), Messages.getString("waters"),
            Messages.getString("dives"));
      status.messageInfo(Messages.getString("calculate_statistic"));
      status.countingProgressbarStart(divelog.getDives().size(), true);
      try {
   ...
   private CountChartData3D getFrequencyYearData() {
      CountChartData3D data = new CountChartData3D(Messages.getString("frequency"), Messages.getString("dives"),
            Messages.getString("years"));
      status.messageInfo(Messages.getString("calculate_statistic"));
      status.countingProgressbarStart(divelog.getDives().size(), true);
   ...
   private CountChartData3D getFrequencyYearTimeData() {
      CountChartData3D data = new CountChartData3D(Messages.getString("divetime"), Messages.getString("divetime"),
            Messages.getString("years"));
      status.messageInfo(Messages.getString("calculate_statistic"));
      status.countingProgressbarStart(divelog.getDives().size(), true);
      GregorianCalendar gc = new GregorianCalendar();
      ...
      try {
         ...
         while (it.hasNext()) {
            ...
            if (dive.getDate() != null) {
               ...
               if (dur != null) {
                  UnitConverter c = new UnitConverter(UnitConverter.getSystem(dive.getUnits()),
   ...
   private void addDivePlaceStatistic() {
      ...
      JSplitPane panel = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, new JScrollPane(getDiveCountryTable(chartPanel)),
            chartPanel);
      ...
      countryTable.addMouseListener(new MouseAdapter() {
         public void mouseClicked(MouseEvent e) {
            if (e.getClickCount() == 1) {
               ...
               targetPanel.add(getChartPanel(
                     getDivePlaceStatisticChart((String) countryTable.getValueAt(countryTable.getSelectedRow(), 0))),
                     BorderLayout.CENTER);
