...
public class TankDetailWindow extends JDialog implements ActionListener {
	...
	public TankDetailWindow(Window parent, MixDatabase db,TankTableModel tankTableModel, Tank tank, int diveSystem, int displaySystem) {
		this(parent, db, tankTableModel,tank, false, diveSystem, displaySystem);
		load();
	}
	...
	public TankDetailWindow(Window parent, MixDatabase db, TankTableModel tankTableModel, int diveSystem, int displaySystem) {
		this(parent, db, tankTableModel, new Tank(), true, diveSystem, displaySystem);
	    this.tank.setGas(new Gas());
	}

	private TankDetailWindow(Window parent, MixDatabase db, TankTableModel tankTableModel, Tank tank, boolean newTank, int diveSystem, int displaySystem ) {
	...
	private void save() {
	    if (getFieldVolume().getText() != null && !"".equals(getFieldVolume().getText())) { //$NON-NLS-1$
	        try {
	            this.tank.getGas().setTankvolume(fromDisplay.convertVolume(Double.valueOf(getFieldVolume().getText())));
	    ...
	    if (getFieldPstart().getText() != null && !"".equals(getFieldPstart().getText())) { //$NON-NLS-1$
	        try {
	            this.tank.getGas().setPstart(fromDisplay.convertPressure(Double.valueOf(getFieldPstart().getText())));
	        } catch (NumberFormatException ex) {
	    ...
	    if (getFieldPend().getText() != null && !"".equals(getFieldPend().getText())) { //$NON-NLS-1$
	        try {
	            this.tank.getGas().setPend(fromDisplay.convertPressure(Double.valueOf(getFieldPend().getText())));
	        } catch (NumberFormatException ex) {
