...
public class SmartAdapter extends TreeSet<JDive> {
   ...
   public SmartAdapter(SmartData aladinData) {
      ...
      for (int index = 0; index < logbook.size(); index++) {
         ...
         dive.setDuration(Double.valueOf(logEntry.duration));
         ...
         depthProfile.setSurfaceinterval(String.valueOf(logEntry.surfaceInterval));
   ...
   private Dive convertDepthProfile(List<DepthProfileEntry> profileEntries) {
      ...
      for (int entryIndex = 0; entryIndex < profileEntries.size(); entryIndex++) {
         ...
         result.addDepth(Double.valueOf(Double.valueOf(profileEntry.depth) / 100).toString());
         result.addTemperature(Double.valueOf(profileEntry.temperature).toString());
         result.addTime(convertTime(profileEntry.time));
      }

      // set the last depth to 0 meters for the profile
      result.addDepth("0");
      result.addTemperature(Double.valueOf(profileEntries.get(profileEntries.size() - 1).temperature).toString());
   ...
   private String convertTime(int seconds) {
      return Double.valueOf(Double.valueOf(seconds) / 60).toString();
   }
