...
public class GalileoSolLogEntry extends LogEntry {
   ...
   public GalileoSolLogEntry(byte[] read_data, int offset) {
      ...
      diveStartTime = getDateFromAladin((int) DiveParser.readULong(read_data, offset + 8));
      ...
      maximumDepth = Float.valueOf(DiveParser.readUInt(read_data, offset + 22)) / 100;
      duration = DiveParser.readUInt(read_data, offset + 26);
      maximumTemperature = Float.valueOf(DiveParser.readInt(read_data, offset + 28)) / 10;
      minimumTemperature = Float.valueOf(DiveParser.readInt(read_data, offset + 30)) / 10;
      airTemperature = Float.valueOf(DiveParser.readInt(read_data, offset + 32)) / 10;
   ...
   private void readAbsoluteDepth() {
      int depth = DiveParser.ByteToU16(new byte[] { read_data[index + 2], read_data[index + 1] });
   ...
   private void readAbsoluteTemperature() {
      temperature = DiveParser.ByteToS16(new byte[] { read_data[index + 2], read_data[index + 1] });
      index += 3;
   }
      ...
      if (available() > 0) {
         ...
         if (dti != 0xFF) {
            if (dti == 0xF0) {
               ...
               // 11110001
               if (available() > GalileoSolDTI.ABSOLUTE_DEPTH.getExtraBytes()) {
                  result = GalileoSolDTI.ABSOLUTE_DEPTH;
               }
            }
            else if (dti == 0xF2) {
               // 11110010
               if (available() > GalileoSolDTI.ABSOLUTE_RBT.getExtraBytes()) {
                  result = GalileoSolDTI.ABSOLUTE_RBT;
               }
            }
            else if (dti == 0xF3) {
               // 11110011
               if (available() > GalileoSolDTI.ABSOLUTE_TEMPERATURE.getExtraBytes()) {
                  result = GalileoSolDTI.ABSOLUTE_TEMPERATURE;
               }
            }
            else if (dti == 0xF4) {
               // 11110100
               if (available() > GalileoSolDTI.ABSOLUTE_TANK_1_PRESSURE.getExtraBytes()) {
                  result = GalileoSolDTI.ABSOLUTE_TANK_1_PRESSURE;
               }
            }
            else if (dti == 0xF5) {
               // 11110101
               if (available() > GalileoSolDTI.ABSOLUTE_TANK_2_PRESSURE.getExtraBytes()) {
                  result = GalileoSolDTI.ABSOLUTE_TANK_2_PRESSURE;
               }
            }
            else if (dti == 0xF6) {
               // 11110110
               if (available() > GalileoSolDTI.ABSOLUTE_TANK_3_PRESSURE.getExtraBytes()) {
                  result = GalileoSolDTI.ABSOLUTE_TANK_3_PRESSURE;
               }
            }
            else if (dti == 0xF7) {
               // 11110111
               if (available() > GalileoSolDTI.ABSOLUTE_HEARTRATE.getExtraBytes()) {
               ...
               // 0ddddddd
               if (available() > GalileoSolDTI.DELTA_DEPTH.getExtraBytes()) {
               ...
               // 1010dddd
               if (available() > GalileoSolDTI.DELTA_TANK_PRESSURE.getExtraBytes()) {
                  result = GalileoSolDTI.DELTA_TANK_PRESSURE;
               }
            }
            else if ((dti & 0xF0) == 0xB0) {
               // 1011dddd
               if (available() > GalileoSolDTI.DELTA_TEMPERATURE.getExtraBytes()) {
               ...
               // 1101dddd
               if (available() > GalileoSolDTI.DELTA_HEARTRATE.getExtraBytes()) {
