...
public class AladinTecLogEntry extends LogEntry {
   ...
   private void readAbsoluteDepth() {
      int depth = DiveParser.ByteToU17(new byte[] { read_data[index + 2], read_data[index + 1], read_data[index + 0] });
   ...
   private void readDeltaDepth2() {
      depth += DiveParser.ByteToS11(new byte[] { read_data[index + 0], read_data[index + 1] });
      index += 2;
      completeSegment();
   }

   private void readDeltaTemperature2() {
      temperature += DiveParser.ByteToS10(new byte[] { read_data[index + 0], read_data[index + 1] });
      index += 2;
   }
   ...
   private AladinTecDTI readDTI() {
      ...
      if (available() > 0) {
         ...
         if (dti != 0xFF) {
            if (dti == 0xFE) {
               // 11111110
               if (available() > AladinTecDTI.ABSOLUTE_TEMPERATURE.getExtraBytes()) {
                  result = AladinTecDTI.ABSOLUTE_TEMPERATURE;
               }
            }
            else if ((dti & 0xFE) == 0xFC) {
               // 1111110d
               if (available() > AladinTecDTI.ABSOLUTE_DEPTH.getExtraBytes()) {
                  result = AladinTecDTI.ABSOLUTE_DEPTH;
               }
            }
            else if ((dti & 0xFC) == 0xF8) {
               // 111110dd
               if (available() > AladinTecDTI.DELTA_TEMPERATURE2.getExtraBytes()) {
               ...
               // 10dddddd
               if (available() > AladinTecDTI.DELTA_TEMPERATURE.getExtraBytes()) {
