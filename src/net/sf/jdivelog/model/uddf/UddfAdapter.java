...
public class UddfAdapter {
    ...
    private Dive buildUdcfDive(UddfFileDive uddfDive, UddfFileGasDefinitions gasDefinitions) throws UddfException {
        ...
        dive.setSurfaceinterval(uddfDive.getSurfaceinterval() == null ? null : uddfDive.getSurfaceinterval().getPassedtime());
    ...
    private void addSamples(Dive dive, List<UddfFileWayPoint> waypoints, UddfFileGasDefinitions gasDefinitions) throws UddfException {
        ...
        while (it.hasNext()) {
            ...
            List<String> sensors = waypoint.getMeasuredpo2Sensors();
            for (String sensor : sensors) {
                PPO2 ppo2 = buildSensorPo2(waypoint, sensor);
                dive.addSample(ppo2);
            }
    ...
    private PPO2 buildSensorPo2(UddfFileWayPoint waypoint, String sensor) {
        PPO2 ppo2 = new PPO2();
        ppo2.setValue(UddfUtilsConverter.getInstance().convertToDouble(waypoint.getMeasuredpo2(sensor)));
        ppo2.setSensor(sensor);
        return ppo2;
    }
