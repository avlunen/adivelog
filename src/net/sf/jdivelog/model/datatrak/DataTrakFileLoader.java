...
public class DataTrakFileLoader {
      ...
      for (int i = 0; i < files.length; i++) {
         try {
            ...
            for (int k = 0; k < count_tg; k++) {
               ...
               line = new byte[4];
               in.read(line);
               count = unsigned_int(line[3]) * 0x1000000 + unsigned_int(line[2]) * 0x10000
                     + unsigned_int(line[1]) * 0x100 + unsigned_int(line[0]);
               cal.add(Calendar.DATE, count);
               // set the dive date
               try {
                  dive.setDate(df.parse(
                        cal.get(Calendar.DATE) + "." + (cal.get(Calendar.MONTH) + 1) + "." + cal.get(Calendar.YEAR)));
                  divedata.setDate(df.parse(
                        cal.get(Calendar.DATE) + "." + (cal.get(Calendar.MONTH) + 1) + "." + cal.get(Calendar.YEAR)));
               ...
               // set the dive time
               dive.setTime(Integer.valueOf(cal.get(Calendar.HOUR_OF_DAY)).toString(),
               ...
               if (dive.getVisibility() != null) {
                  dive.setVisibility(
                        dive.getVisibility() + " m  " + Messages.getString("weather") + ": " + help_string);
               }
               else {
               ...
               // set tank size
               if (count != 32767) {
                  divegas.setTankvolume(Double.valueOf(count) / 100000);
               }
               ...
               if (count != 32767) {
                  dive.setDepth(Double.valueOf(count) / 100);
               }
               ...
               // set dive duration
               if (count != 32767) {
                  dive.setDuration(Double.valueOf(count));
               }
               ...
               if (count != 32767) {
                  dive.setTemperature(Double.valueOf(count) / 100);
                  divedata.setTemperature(Double.valueOf(count) / 100);
               }
               ...
               // set air consumption
               if (count != 32767) {
                  divegas.setPstart(Double.valueOf(Messages.getString("standard_pressure")));
                  divegas.setPend(divegas.getPstart() - (Double.valueOf(count) / 100));
               }
               ...
               if (count != 0) {
                  line = new byte[count];
                  in.read(line);
                  divesecond = Double.valueOf(0);
                  divedata.addTime(divesecond.toString());
                  divedata.addDepth("0");
                  divedata.addGas(divegas);
                  for (int j = profiledataOffset; j < count - profiledataPacketLength; j = j
                        + profiledataPacketLength) {
                     depth = Double.valueOf((unsigned_int(line[j + 1]) * 0x100 + unsigned_int(line[j])) & 0xffc0) / 410;
                     divedata.addTime(Double.valueOf(divesecond / 60).toString());
                     divedata.addDepth(depth.toString());
                     setAlarms(line[j + 1], line[j], workAlarms);
                     divesecond = divesecond + 20;

                     depth = Double.valueOf((unsigned_int(line[j + 3]) * 0x100 + unsigned_int(line[j + 2])) & 0xffc0)
                           / 410;
                     divedata.addTime(Double.valueOf(divesecond / 60).toString());
                     divedata.addDepth(depth.toString());
                     ...
                     depth = Double.valueOf((unsigned_int(line[j + 5]) * 0x100 + unsigned_int(line[j + 4])) & 0xffc0)
                           / 410;
                     divedata.addTime(Double.valueOf(divesecond / 60).toString());
                     divedata.addDepth(depth.toString());
   ...
   /**
    * convert characters encoded with CP850 character set into a Java (unicode)
    * string
    *
    * @param input Description of the Parameter
    * @return Description of the Return Value
    */
   public static String characterGerman(byte[] input) throws UnsupportedEncodingException {
