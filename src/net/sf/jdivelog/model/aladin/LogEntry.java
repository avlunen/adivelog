...
public class LogEntry {
   ...
   public LogEntry(int[] bytes, int aladinType) {
      this(bytes, aladinType, 0);
   }

   public LogEntry(int[] bytes, int aladinType, int timeAdjustment) {
      if ((bytes == null) || (bytes.length < DATA_SIZE)) {
         throw new IllegalArgumentException("need " + DATA_SIZE + " bytes for log entry");
      }
      alarms = bytes[0];
      ...
      entryTime = getEntryTime(bytes[7], bytes[8], bytes[9], bytes[10], timeAdjustment);
      waterTemperature = Float.valueOf((bytes[11] * 25) / 100.0f);
   }
   ...
   public static Date getEntryTime(int byte1, int byte2, int byte3, int byte4, int timeAdjustment) {
      ...
      calendar.add(Calendar.SECOND, time + timeAdjustment);
   ...
   private String getHighPlace() {
      switch ((alarms & 0xc0) >> 6) {
         ...
         case 1:
            return "900 m - 1700 m";
         case 2:
            return "1700 m - 2700 m";
         case 3:
            return "2700 m - 4000 m";
         default:
            return "error calculating high place value";
      }
   }
