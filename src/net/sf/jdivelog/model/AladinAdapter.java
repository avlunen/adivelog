...
public class AladinAdapter extends TreeSet<JDive> {
   ...
   public AladinAdapter(AladinData aladinData) {
      ...
      List<Dive> depthProfiles = convertDepthProfiles(aladinData.getDepthProfiles());
      ...
      for (int index = 0; index < logbook.size(); index++) {
         ...
         if (logbook.size() - index <= depthProfiles.size()) {
            Dive depthProfile = depthProfiles.get(depthProfiles.size() + index - logbook.size());

            depthProfile.addGas(gas);
            depthProfile.setDate(logEntry.entryTime);
            depthProfile.setSurfaceinterval(String.valueOf(logEntry.surfaceTime));
            dive.setDate(logEntry.entryTime);
   ...
   private List<Dive> convertDepthProfiles(List<DepthProfile> profiles) {
      ...
      for (int index = 0; index < profiles.size(); index++) {
         ...
         dive.setDensity(Double.valueOf(0));
         dive.setAltitude(Double.valueOf(0));
         dive.setTemperature(Double.valueOf(profile.startTemperature));
         dive.addTime(getNextDiveTime());
         dive.addDepth("0");

         List<DepthProfileEntry> profileEntries = profile.getProfileEntries();

         for (int entryIndex = 0; entryIndex < profileEntries.size(); entryIndex++) {
            final DepthProfileEntry profileEntry = profileEntries.get(entryIndex);
         ...
         dive.addDepth(Double.valueOf(0).toString());
         dive.addTime(getNextDiveTime());
   ...
   public static Gas getGas(int airConsumption) {
      ...
      result.setOxygen(Double.valueOf(0.21));
      result.setNitrogen(Double.valueOf(0.79));
      result.setHelium(Double.valueOf(0));
