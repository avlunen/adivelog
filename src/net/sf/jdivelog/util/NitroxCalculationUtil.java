...
public class NitroxCalculationUtil {
    ...
    * @return percentage of the maximum exposure for one dive. <code>null</code> if
    *         the partial pressure is over 1.6 bar (not in table allowed)
    */
   public static Double getSingleDiveCNS(Double ppo, Double minutes) {
      int ppo2 = Double.valueOf(Math.ceil(ppo * 10)).intValue();
      if (ppo2 < 6) {
         return 0.0;
      }
      if (ppo2 > 16) {
    ...
    * @param ppo     partial pressure in bar.
    * @param minutes time of exposure in minutes.
    * @return percentage of the maximum exposure per day. <code>null</code> if the
    *         partial pressure is over 1.6 bar (not in table allowed)
    */
